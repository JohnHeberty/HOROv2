...
def calcular_fo(
    ...
    """
    Calcula o Fator de Operação (FO) para uma orientação de pista.

    O vento cruzado é a componente perpendicular ao eixo da pista.
    FO = % de observações com vento cruzado ≤ crosswind_limit_kts.
    ...
    Args:
        ...
        crosswind_limit_kts: Limite de vento cruzado (padrão RBAC154: 20 kt).
    ...
    # Somente ventos não-calmos para o cálculo de componente
    if keep_calms:
        wind_data = df[df["spd"] > 0]
    else:
        wind_data = df

    # Ângulo relativo à pista
    rel_angle_rad = np.radians(wind_data["dir"] - heading_deg)
    crosswind_component = np.abs(wind_data["spd"] * np.sin(rel_angle_rad))

    inside = crosswind_component <= crosswind_limit_kts
    n_inside = inside.sum()
    n_calms  = len(calms) if keep_calms else 0

    fo_pct         = (n_inside + n_calms) / total * 100
    crosswind_pct  = (total - n_inside - n_calms) / total * 100
